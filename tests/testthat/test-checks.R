...
test_that("Checks for proper TRUE or FALSE work", {
  expect_true(is_TRUE_FALSE(TRUE))
  expect_error(is_TRUE_FALSE(1), "must be")
  expect_error(is_TRUE_FALSE("TRUE"), "must be")
})

test_that("Checks for proper index work", {
  expect_true(is_index(1))
  expect_error(is_index(0), "must be")
  expect_error(is_index("one"), "must be")
})
