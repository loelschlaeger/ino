...
mixture_ino$optima(digits = 0, sort_by = "value")


## ---- check assumptions about optimization results, include = FALSE, eval = TRUE----------------------------
optima <- mixture_ino$optima(digits = 0, sort_by = "value")
stopifnot(as.numeric(as.character(optima[1, "value"])) == 276)
most_occuring <- mixture_ino$optima(digits = 0, sort_by = "frequency")[1:2, ]
...
mixture_ino$optima(digits = 0, sort_by = "value", which_optimizer = "nlm")
mixture_ino$optima(digits = 0, sort_by = "value", which_optimizer = "optim")
mixture_ino$optima(digits = 0, sort_by = "value", which_optimizer = "em")


## ---- closest parameters, eval = TRUE-----------------------------------------------------------------------
(mle <- mixture_ino$closest_parameter(value = 276, which_optimizer = "nlm"))
mixture_ino$evaluate(at = as.vector(mle))
mle_run <- attr(mle, "run")
(bad <- mixture_ino$closest_parameter(value = 421, which_optimizer = "nlm"))
...
## ---- check assumptions about optimization results 2, include = FALSE, eval = TRUE--------------------------
noptima_new <- nrow(mixture_ino$optima(digits = 0, sort_by = "value", which_run = "custom_sampler"))
noptima_old <- nrow(mixture_ino$optima(digits = 0, sort_by = "value", which_run = "random"))
stopifnot(noptima_new > noptima_old)
most_occuring <- mixture_ino$optima(digits = 0, sort_by = "frequency", which_run = "custom_sampler")[1, ]
most_occuring_value <- as.numeric(as.character(most_occuring$value))
...
## ---- bad educated guess------------------------------------------------------------------------------------
mixture_ino$optimize(initial = rep(0, 5), label = "bad_educated_guess")


## ---- bad educated guess summary, eval = TRUE---------------------------------------------------------------
summary(mixture_ino, which_run = "bad_educated_guess")


## ---- standardize data, eval = TRUE-------------------------------------------------------------------------
...
mixture_ino$plot(by = "label", relative = TRUE)


## ---- plot-by-optimizer, eval = TRUE------------------------------------------------------------------------
mixture_ino$plot(by = "optimizer", relative = FALSE)
