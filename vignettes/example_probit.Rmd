...
X <- function(n, t) {
  ...
  cost <- round(stats::rlnorm(3, meanlog = 7, sdlog = 0.3))
  
  set.seed(n)
  age_group <- sample(c("young", "old"), size = 1)
  
  age <- (age_group == "young") * rpois(n = 1, lambda = 30) + (age_group == "old") * rpois(n = 1, lambda = 60)
  
  
  covariates <- cbind(cost, diag(3)[, -1] * age)
  rownames(covariates) <- c("beach", "cruise", "party")
  colnames(covariates) <- c("cost", "age_cruise", "age_party")
  return(covariates)
=======
...
Omega <- Sigma
probit_data <- sim_mnp(N, T, J = 3, P = 3, b, Omega, Sigma, X, seed = 1)
=======
...
probit_ino <- Nop$new(f = f_ll_mnp, npar = length(theta), data = probit_data, neg = TRUE)$
