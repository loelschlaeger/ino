...
The {ino} package provides the `sim_mnp()` function for simulating choice data from a probit model. To use this function, we need to define a function `X()` that samples the matrix of choice characteristics for the decision maker $n$ at choice occasion $t$. The matrix should have dimensions of $J \times P$, where $J$ is the number of available alternatives and $P$ the number of characteristics that describe each alternative.
