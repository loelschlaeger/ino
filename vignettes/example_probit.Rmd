...
In the probit model formulation, a total of $N$ deciders are faced with choosing among $J \geq 2$ alternatives at each of the $T$ choice occasions. The choice made by decider $n$ at occasion $t$ is denoted as $y_{nt}$ and can take values in the set $\{1, \dots, J\}$. For more comprehensive details on the probit model and its estimation, please refer to the works by @Train:2009 and @Bhat:2011.
...
The following function enables simulation of choice data from a probit model:

```{r, define function for data simulation from probit model, eval = TRUE}
sim_mnp <- function(
...
The probit likelihood function is implemented as follows:

```{r, define function for probit likelihood evaluation, eval = TRUE}
f_ll_mnp <- function(
...
The likelihood can evaluated by providing a parameter vector `theta` and the data set `probit_data`:
...
- We first define a `Nop` object by setting the target function `objective = f_ll_mnp`, the number of parameters to `npar = 7`, and `data = probit_data`. 
...
Nop_probit$
  ...
  reduce("data", how = "random", proportion = 0.2)$
  initialize_random(runs = 100)$
  optimize(label = "standardized_reduced")$
  fixed_argument("reset", argument_name = "data")$
  standardize("data", by_column = TRUE, ignore = 1:3)$
  initialize_continue(which_run = "standardized_reduced")$
  optimize(label = "initialized_standardized_reduced")
```
...
plot(Nop_probit, by = "label", relative = TRUE, xlim = c(-1, 3))
```
