...
To specify an optimization problem in {ino}, we use an object-oriented framework based on the package {R6} [@Chang:2021]. The general workflow is to first create
...
TODO: reference to Zucchini and Hastie book which have done the same?
...
The `faithful` data set contains information about eruption times of the Old Faithful geyser in Yellowstone National Park, Wyoming, USA.
...
The `$optima()` method provides an overview of the identified optima (we ignore the decimal places by setting `digits = 0`):
...
The 100 optimization runs with 2 optimizers using random starting values led to `r noptima` different optima (minima in this case, because we minimize `normal_mixture_llk()`). Hence we can deduce that the initial starting values have a huge impact on the optimization result.

The two most occurring optimum values are `r paste(most_occuring_value, collapse = " and ")` with frequencies `r paste(most_occuring_frequency, collapse = " and ")`, respectively. The value 276 is the overall minimum (potentially the global minimum), while 421 is clearly worse. To compare the parameter vectors that led to these different values, we can use the `$which_parameter()` method:
...
The parameter vectors are denoted as `mle` (this is our maximum likelihood estimate) and `bad` (this clearly is a bad estimate). To understand the values in terms of means, standard distribution, and mixing proportion (i.e., in the form $\boldsymbol{\theta} = (\mu_1, \mu_2, \sigma_1, \sigma_2, \pi)$), they need transformation (see above):
...
They correspond to the following mixture densities:

```{r}
mixture_density <- function (x, mu, sigma, pi) {
...
ggplot(faithful, aes(x = eruptions)) + 
  ...
  stat_function(
    fun = function(x) {
      mixture_density(x, mu = mle[1:2], sigma = mle[3:4], pi = mle[5])
    }, aes(color = "mle"), linewidth = 1
  ) +
  stat_function(
    fun = function(x) {
      mixture_density(x, mu = bad[1:2], sigma = bad[3:4], pi = bad[5])
...
To obtain the first results of these optimization runs, we can use  the `summary()` method:

```{r}
summary(geyser, which_runs = "custom_sampler") |>
  head(n = 10)
```

We still obtain different optima, but (in contrast to the example above) most of the runs here lead to the optimum of `r summary(geyser, c("value", "parameter", "label")) %>%  dplyr::filter(label == "custom_sampler") %>% dplyr::count(value) %>% dplyr::pull(value) %>% .[1] %>% round()`:
...
```{r}
starting_values <- list(c(1.8, 4.2, log(1), log(1), qlogis(0.5)),
                        c(2.2, 3.8, log(1), log(1), qlogis(0.5)))
```

For comparison, we also consider a third set of starting values which are somewhat implausible.

```{r}
starting_values[[3]] <- rep(0, 5)
```
...
```{r}
summary(geyser, which_runs = "educated_guess") 
```

TODO: also round parameters
...
For standardized initialization, we standardize all columns in our data before running the optimization. The method `$standardize()` thus requires the name of the data set to be standardized --- in our case `data`. To optimize the likelihood using the standardized data set, we again use `optmize()`, which by default uses random starting values. Here, we could again specify the distribution from which we draw the random initial values, or we could of course also set fixed starting values.   
...
To perform further optimization runs after having applied standardized initialization, we use `reset_argument("data")` to unstandardize the data and obtain the actual data set we considered before. If we do not use `reset_argument()`, all further optimizations runs will be carried out on the standardized data set. 
...
In addition to selecting sub samples at random, we can specify four other options using the argument `how`:
...
We can also create that these boxplots manually by using the data.frame provided by `summary()` and sort the boxplots by their medians:
```{r}
summary(geyser, c("label", "seconds")) %>% 
  ggplot(aes(x = reorder(label, seconds, median), y = seconds)) + 
           geom_boxplot() + xlab("")
```
...
TODO: We need to be a bit careful here because of `standardize()`
