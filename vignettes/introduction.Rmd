...
where $\phi_{\mu_1, \sigma_1^2}$ and $\phi_{\mu_2, \sigma_2^2}$ denote the normal density for the first and second cluster, respectively, and $\lambda$ is the mixing proportion. The vector of parameters to be estimated is thus $\boldsymbol{\theta} = (\mu_1, \mu_2, \sigma_1, \sigma_2, \lambda)$. As there exists no closed-form solution for the maximum likelihood estimator $\boldsymbol{\theta}^* = \arg\max_\boldsymbol{\theta} \ell(\boldsymbol{\theta})$, we need numerical optimization for finding the function optimum. 
...
The following function implements this algorithm, which we will compare to standard numerical optimizers below.
...
em <- function(normal_mixture_llk, theta, epsilon = 1e-08, iterlim = 1000, data) {
  ...
  lambda <- plogis(theta[5])
  for (i in 1:iterlim) {
    gamma_1 <- lambda * dnorm(data, mu[1], sd[1])
    gamma_2 <- (1 - lambda) * dnorm(data, mu[2], sd[2])
    posterior <- gamma_1 / (gamma_1 + gamma_2)
...
- and `data` gives the observations as required by our likelihood function. 
...
Additionally, we want to apply the EM algorithm introduced above:

```{r}
em_optimizer <- optimizeR::define_optimizer(
  optimizer = em,
  objective = "normal_mixture_llk",
  initial = "theta",
  value = "neg_llk",
  parameter = "estimate",
  validation_settings = list(
    objective_test = normal_mixture_llk,
    objective_add = list("data" = faithful$eruptions),
    initial = 1:5
  )
)
...
Finally, we recommend to test the specification:
...
### check if one fail entry exists
stopifnot(sum(is.na(optima$value)) == 1)
### check if seven runs failed
stopifnot(optima[which(is.na(optima$value)), "frequency"] == 7)
### check number of different optima
noptima <- length(optima$value[!is.na(optima$value)])
stopifnot(noptima == 23)
### most occuring optima are 421 and 276
...
### frequencies of values 421 and 276 did not change
most_occuring_frequency <- most_occuring$frequency
stopifnot(most_occuring_frequency == c(159, 112))
```
...
The two most occurring optima are 421 and 276 with frequencies 159 and 112, respectively. The value 276 is the overall minimum (potentially the global minimum), while 421 is significantly worse. To compare the parameter vectors that led to these different values, we can use the `$which_parameter()` method. From the saved optimization runs, it extracts the parameter vector corresponding to an optimum closest to `value`:
...
It becomes very clear that the mixture defined by `mle` fits much better than `bad`, which practically estimates only a single class.
...
stopifnot(min(most_occuring_value, na.rm = TRUE) == 276)
```

We still obtain different optima, even more than before. But in contrast, most of the runs here lead to the presumably global optimum of 276:
...
For comparison, we consider a set of implausible starting values, which led to local optima:
...
In some situations, it is possible to consider a standardized version of the optimization problem, which could potentially improve the performance of the numerical optimizer. In our example, we can standardize the geyser data before running the optimization via the `$standardize()` method:
...
```{r}
geyser$
  optimize(runs = 100, label = "data_subset_random")$
  reset_argument("data")$
  continue()
```
...
The `$plot()` method provides an overview of the optimization times. Setting `"by = "label"` allows comparison across initialization strategies, setting `"relative = TRUE"` plots relative differences to the median of the top boxplot:
...
```{r}
summary(geyser, c("label", "value", "optimizer")) %>% 
  ggplot(aes(x = label, y = value, color = optimizer)) + 
