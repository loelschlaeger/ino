...
Whenever we numerically optimize a function, the optimization algorithm and chosen starting values can have a substantial effect on the computational cost or even on the results. The purpose of the {ino} package is to provide a comprehensive toolbox for comparing such different settings when performing numerical optimization. 

In the following, we demonstrate the main functionality of {ino}. In particular, we show how to numerically optimize a likelihood function using {ino}, thereby applying different strategies for the choice of the starting values and the optimization strategy.

## Example: Fitting a Gaussian mixture using {ino}

We consider the popular `faithful` data set that is provided via base R.^[The data set contains information about the eruption times of the Old Faithful geyser in Yellowstone National Park, Wyoming, USA: the eruption time and the waiting time to the next eruption (both given in minutes).] The histogram of eruption times indicate two clusters with short and long eruption times, respectively:
...
For both clusters, we assume a normal distribution here, such that we consider a mixture of two normal distributions for the eruption times. The log-likelihood is then given by
...
<!-- The messages printed in the console summarizses that we are using fixed initialization here. In addition, there is also information given about a grid --- this concerns the progress if, for example, multiple data sets are used. We will learn more about this below. -->

Before we evaluate the optimization runs, we first add some further optimization runs using randomly chosen starting values. 
...
The `geyser_ino` object now contains the results and further information on five optimization runs (three runs with fixed and two with randomly selected starting values). As we have chosen one rather unplausible set of fixed starting values, we first delete this (i.e., the third) run:
...
Here, the first run with fixed chosen starting values requires `r summary(geyser_ino, group = NULL)[1,8]` iterations until convergence --- much less than the optimizations with randomly chosen starting values. The final (negative) log-likelihood value (column `minimum`) further indicates that the optimization run with random starting values drawn from a $\mathcal{N}(0,1)$ distribution (the third row) resulted in a local minimum.
