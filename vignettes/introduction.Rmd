...
To specify an optimization problem in {ino}, we use an object-oriented framework based on the package R6. The general workflow is to first create
...
The `faithful` data set contains information about eruption times of the Old Faithful geyser in Yellowstone National Park, Wyoming, USA. The following histogram hints at two clusters with short and long eruption times, respectively:
...
The `$optima()` method provides an overview of the identified optima. We see below that the different runs led to `r nrow(geyser$optima(digits = 0, sort_by = "value"))` different optima (minima in this case, because we minimize `normal_mixture_llk()`), hence the initial starting values have a huge impact on the optimization result.
...
To obtain the results of these optimization runs, we can use `summary()` to obtain (e.g.) the optima and the initial parameters. Together with functions from the dplyr package, we access only the optimization runs of the `custom_sampler`: 

```{r}
summary(geyser, c("value", "label")) %>% 
  dplyr::filter(label == "custom_sampler") %>% 
  head(n = 10)
```

Here, we still obtain different optima, but most of the runs lead to the optimum of `r summary(geyser, c("value", "parameter", "label")) %>%  dplyr::filter(label == "custom_sampler") %>% dplyr::count(value) %>% dplyr::pull(value) %>% .[1] %>% round()`:

```{r}
summary(geyser, c("value", "label")) %>%  
  dplyr::filter(label == "custom_sampler") %>% 
  dplyr::mutate(value_rounded = round(value)) %>% 
  dplyr::count(value_rounded)
```
...
```{r}
summary(geyser, c("value", "seconds", "label")) %>% 
  dplyr::filter(label == "educated_guess")
```

TODO: apply round to reduce decimal places

The `summary()` method returns an overview of the three optimization runs performed with the two optimizer. In addition to the optima, the optimization time and the label, `summary()` can also show the estimated parameters for all optimization runs:

```{r}
summary(geyser, c("value", "parameters", "seconds", "label")) %>% 
  dplyr::filter(label == "educated_guess")
```

With `geyser$summary_columns`, the names of all available columns of the `summary()` output are shown. 


```{r}
# geyser$get_value(which_value = "initial", which_runs = 10, which_optimizer = "nlm")
```
