...
- For evaluating the results, `$optima()` prints an overview of all identified optima, and the `$plot()` and `$summary()` methods summarize the optimization runs.
...
The optimization problem is specified as a `Nop` object called `geyser` below, where
...
In addition to the numerical optimizers, we also want to apply the EM algorithm introduced above:
...
Looking at this overview optimizer-wise reveals that the `stats::optim` optimizer is most vulnerable to local optima:
...
The two most occurring optima are 421 and 276 with total frequencies of `r geyser$optima(digits = 0)[which(as.numeric(as.character(geyser$optima(digits = 0)$value)) == 421), "frequency"]` and `r geyser$optima(digits = 0)[which(as.numeric(as.character(geyser$optima(digits = 0)$value)) == 276), "frequency"]`, respectively. The value 276 is the overall minimum (potentially the global minimum), while 421 is significantly worse. To compare the parameter vectors that led to these different values, we can use the `$which_parameter()` method. From the saved optimization runs, it extracts the parameter vector corresponding to an optimum closest to `value`:
