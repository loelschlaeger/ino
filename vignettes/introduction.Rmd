...
Whenever we numerically optimize a function, we need to take care of several things: the chosen starting values, optimization algorithm, and optimizsation strategy (such as subset optimization) can have a substantial effect on the computational cost or even on the results. The purpose of the {ino} package is to provide a comprehensive toolbox for comparing such different settings when performing numerical optimization. 
