...
The final parameter estimates (i.e., the parameters associated with the global optimum) can be accessed:

```{r, best parameter, eval = TRUE}
Nop_hmm$minimum
```

Next, we can compute the proportion of runs that lead to the apparent global optimum as follows (note that the standardized initialization approach cannot be compared to the other approaches here):
...
The `autoplot()` method can be used to investigate the optimization times across initialization strategies (`group_by = ".optimization_label"`).^[Setting `relative = FALSE` in the `plot()` call would show the absolute times.]  
