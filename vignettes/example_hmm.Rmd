...
db_data <- data %>%
  as_tibble() %>%
  reframe(date = as.Date(Date, format = "%Y-%m-%d"),
...
As the share returns are continuous and can take both negative and positive values, we consider an HMM with Gaussian state-dependent distributions. The (log-)likelihood of a Gaussian-HMM is given by the function `f_ll_hmm()`.
