...
description: >
  The {ino} package in action for HMM likelihood optimization.
...
HMMs are statistical models used to explain systems that can only be observed indirectly through a sequence of outputs, and have unobservable hidden states. They consist of two processes, a Markov process that describes the hidden states, and an observable process that describes the outputs produced by the system, where the probability of observing an output at a given time depends only on the current state. State-switching models like HMMs are commonly used in speech recognition, animal movement modeling, and finance. For more technical details about HMMs and their scope of application, see, e.g., @Zucchini:2016.

Numerical likelihood optimization is a prominent method for fitting HMMs to empirical data. In this vignette^[The vignette was build using R `r paste(R.Version()[c("major","minor")], collapse = ".")` with the `{ino}` `r utils::packageVersion("ino")` package.] we describe the workflow of using the `{ino}` package for analyzing the initialization effect in this optimization task. 

## Application to financial data

The example data set considered throughout this vignette covers a time series of log returns from the [German stock index DAX](https://en.wikipedia.org/wiki/DAX) over 30 years. The DAX closing prices are freely accessible via [Yahoo Finance](https://finance.yahoo.com) and can be downloaded via the `download_data()` function from the `{fHMM}` package [@Oelschläger:2023b]. We transform them to log-returns using the `{dplyr}` package [@Wickham:2023]:
...
For convenience, The `{ino}` package supports parallel computation of optimization runs based on the `{future}` package [@Bengtsson:2021] and printing progress messages based on the `{progressr}` package [@Bengtsson:2023]. 
...
Using `summary()`, we now can investigate the optimum values (`"value"`), the corresponding parameter vectors (`"parameter"`), and the optimization times (`"seconds"`) of all runs (here, only the first ten are shown):
...
In addition, the `elements()` method provides an overview of all available optimization result elements:

```{r, elements available, eval = TRUE}
Nop_hmm$elements()
```

The final parameter estimates (i.e., the parameters associated with the global optimum) can be accessed via `best("parameter")` ...
...
Note that the attributes `".run_id"` and `".optimizer_label"` provide the information, in which optimization run and with which optimizer this result was obtained.

Next, we can compute the proportion of runs that lead to the apparent global optimum of `r format(as.numeric(Nop_hmm$best("value")), scientific = FALSE)` as follows (note that the standardized initialization approach cannot be compared to the other approaches here):
...
The `plot()` method can be used to investigate the optimization times across initialization strategies (`group_by = ".optimization_label"`).^[Setting `relative = FALSE` in the `plot()` call would show the absolute times.]  

```{r, optimization-time, eval = TRUE, warning = FALSE, message = FALSE}
Nop_hmm$plot(
