...
- `sort_by = "value"` sorts the table by function value (in decreasing order^[By default, optimization is assumed to involve minimizing the target function. If instead the function is maximized, set `hmm_ino$show_minimum <- FALSE` to reverse the ordering in the `$optima()` output.]),
...
The `plot()` function can be used to investigate the optimization times across initialization strategies (`by = "label"`). Since the absolute times depend  ^[Setting `relative = FALSE` in the `plot()` call shows the absolute times.]  

```{r, optimization time, eval = TRUE}
plot(hmm_ino, by = "label")
```

Using the output provided by `summary()` and some data manipulation functions provided by the package {dplyr}, we can also compute point statistics of interest, like the median computation time per strategy:
