...
We first use randomly chosen starting values. Note that running the following code snippet may take a few minutes, as the optimization is performed with 50 different sets of starting values. The results of the 50 runs are contained in the example ino object `example_hmm.RData` and can be loaded into the workspace via `load(example_HMM.RData)`. 
...
The frequency table indicates that `r overview_optima(hmm_ino)[1,2]` runs converged to the same likelihood value (`r as.numeric(as.character(overview_optima(hmm_ino)[1,1]))`) --- the global maximum is 342.32 (note these are the negative log-likelihood values).  
...
```{r}
which(summary(hmm_ino)$.optimum < 343)
```
...
```{r}
get_vars(hmm_ino, runs = which(summary(hmm_ino)$.optimum < 343)[1])[[1]]$.init
```
...
Using `summary()`, we can also compute the average time per strategy:

```{r}
summary(hmm_ino)
```

This comparison can be considered somehow 'unfair', as not all optimization runs lead to the global optimum --- optimization runs that lead to local optima may then have less iterations. We can first check which runs lead to the global optimum of 342.32. For that, we will use the data manipulation functions provided by the package `dplyr`.

```{r}
summary(hmm_ino) %>% 
  mutate(global_optimum = ifelse(.optimum < 343, 1, 0)) %>% 
  group_by(.strategy) %>% 
  summarise(proportion_global_optimum = mean(global_optimum))
```

For the subset approach, only `r summary(hmm_ino) %>% mutate(global_optimum = ifelse(.optimum < 343, 1, 0)) %>% group_by(.strategy) %>% summarise(proportion_global_optimum = mean(global_optimum)) %>% filter(.strategy == "subset(first,0.5)") %>% pull(proportion_global_optimum) * 100`\% of the runs lead in the global optimum!
...
```{r}
# save(hmm_ino, file = "example_hmm.RData")
```
