...
As the log returns are continuous and can take both negative and positive values, we consider an HMM with Gaussian state-dependent distributions --- note that some applications instead use symmetric lambda or t-distributions to also model the kurtosis [@Lihn:2017; @Oelschläger:2021].
...
Using the output provided by `summary()` and some data manipulation functions provided by the package {dplyr}, we can also compute summary statistics of interest, like the median computation time per strategy:
...
Let's again compare the median computation time, but only for those runs that lead to the apparent global optimum:

```{r, eval = TRUE}
summary(hmm_ino, c("label", "value", "seconds")) %>% 
  filter(value < -22445) %>% 
  ggplot(aes(y = label, x = seconds)) +
  geom_boxplot() + ylab("")
```

Looking at the boxplots, the subset last strategy appears to be considerably slower  than the subset first method. Examining the initial values (obtained from fitting the model to the subset in the first step), for example for the second run,

```{r}
hmm_ino$results("subset_last")[[2]]$initial
# to obtain the inital values of all optimization runs, run
# lapply(hmm_ino$results("subset_last"), '[[', "initial")
```

we see that the starting values for the transition probability matrix (i.e., the first two entries) deviate from the values obtained at the global optimum (cf. `hmm_ino$best_parameter()`). For the subset first strategy, the initial values of the tpm are much closer to those at the global optimum (again considering the second run for illustration):

```{r}
hmm_ino$results("subset_first")[[2]]$initial
```
