...
library("ggplot2")
## download the data
db_data <- read.csv("https://query1.finance.yahoo.com/v7/finance/download/DB?period1=948499200&period2=1663804800&interval=1d&events=history&includeAdjustedClose=true")
db_data$date <- as.Date(db_data$Date, format = "%Y-%m-%d")
## calculate log-returns (multiplied by 100)
db_data$obs <- c(NA, log(db_data$Adj.Close[2:nrow(db_data)] / 
                                   db_data$Adj.Close[1:(nrow(db_data) - 1)])) * 100
db_data <- db_data[-1, ]
## plot the time series of share returns
ggplot(db_data, aes(x = date, y = obs)) +
  geom_point() + geom_line() + ylab("log-returns [%]")
```
...
```{r, eval = FALSE}
hmm_ino <- random_initialization(hmm_ino, runs = 50,
                                 sampler = function() c(stats::rnorm(2, -1.5, 0.5),
                                                        stats::rnorm(2),
...
As the time series of share returns consists of `r nrow(db_data)` observations, we can even try to use fewer observations to fit our model. In the next step, we consider only the first 25% of the observations.

```{r, eval = FALSE}
hmm_ino <- subset_initialization(hmm_ino, arg = "data", how = "first", prop = 0.25,
                                 initialization = 
                                   random_initialization(
                                     runs = 50,
                                     sampler = function() c(stats::rnorm(2, -1.5, 0.5),
...
To further (potentially) decrease the computational cost associated with model fitting, we now consider only the first 5% of the data:

```{r, eval = FALSE}
hmm_ino <- subset_initialization(hmm_ino, arg = "data", how = "first", prop = 0.05,
                                 initialization = 
                                   random_initialization(
                                     runs = 50,
                                     sampler = function() c(stats::rnorm(2, -1.5, 0.5),
...
```{r}
which(summary(hmm_ino)$.optimum < 12931)
```

We can access the corresponding initial values of, for example, the first of these runs via:

```{r}
get_vars(hmm_ino, runs = which(summary(hmm_ino)$.optimum < 12931)[1])[[1]]$.init
```
...
This comparison can however be considered somehow 'unfair', as not all optimization runs lead to the global optimum --- optimization runs that lead to local optima may then have less iterations and hence lower computation time. We can first check which runs lead to the apparent global optimum of 12930.73. For that, we will again use functions from `dplyr`.

```{r}
summary(hmm_ino) %>% 
  mutate(global_optimum = ifelse(.optimum < 12931, 1, 0)) %>% 
  group_by(.strategy) %>% 
...
While computation time is lowest for the fixed initialization strategy, subset initialization appears to be a promising approach. In particular, when considering the first 5% or 25% of the data in a first step, the computation time is most likely to be lower compared to the random initialization approach.
