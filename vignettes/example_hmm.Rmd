...
We first use randomly chosen starting values. Note that running the following code snippet may take a few minutes, as the optimization is performed with 100 different sets of starting values. The results of the 100 runs are contained in the example ino object `example_hmm.RData`. 

```{r}
hmm_ino <- random_initialization(hmm_ino, runs = 3)
```
...
To see which starting values lead to the global optimum, we can access the initial values stored under `hmm_ino$runs$pars`.


```{r}
get_vars(hmm_ino, runs = 4)
```
...
```{r}
summary(hmm_ino, group = ".strategy", "mean_time" = mean(.time))
```
...
For the subset approach, only 7\% of the runs lead in the global optimum!
