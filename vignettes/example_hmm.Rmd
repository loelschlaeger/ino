...
The example data set considered throughout this vignette covers a time series of share returns from the Deutsche Bank stock. The data set is freely accessible via Yahoo Finance and can be downloaded via the {fHMM} package [@Oelschläger:2023b]: 
...
As the share returns are continuous and can take both negative and positive values, we consider an HMM with Gaussian state-dependent distributions. The (log-) likelihood function of a Gaussian-HMM is provided by the {ino} package as the function `f_ll_hmm()`.
...
```{r, eval = FALSE}
hmm_ino <- Nop$new(f = f_ll_hmm, npar = 6, data = db_data, N = 2, neg = TRUE)$
...
```{r, eval = FALSE}
tpm_entry_1 <- tpm_entry_2 <- c(-2, -1.5)
mu_1 <- c(0, -0.1)
mu_2 <- c(0, 0.1)
sd_1 <- c(log(2), log(1))
sd_2 <- c(log(3), log(1.5))

starting_values <- asplit(expand.grid(tpm_entry_1, tpm_entry_2, 
                                      mu_1, mu_2, sd_1, sd_2), MARGIN = 1)

for(val in starting_values)
  hmm_ino$optimize(initial = val, save_results = TRUE, label = "educated_guess")
```
...
We use the `plot()` function to investigate the computation time. Intuitively, optimization runs our educated guesses be faster than with random starting values, since we have carefully chosen these starting values by investigating the data. The boxplots confirm this intuition: 
...
While for the educated guesses starting values all runs converge to the global optimum, the random initialization and subset initialization strategies do sometimes get stuck in local maxima.
