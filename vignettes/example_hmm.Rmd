...
To illustrate the subset initialization strategy, we fit our HMM to the first 50\% and 60\% of the observations, respectively. The starting values for these subsets are chosen randomly. The function `subset_initialization()` then fits the HMM again to the entire sample using the estimates obtained from the subsets as initial values.

```{r}
props <- c(0.5, 0.6)
for(i in 1:length(props))
  hmm_ino <- subset_initialization(hmm_ino, arg = "data", how = "first", prop = props[i])
```
...
When comparing the likelihood values across the strategies, we see that the random starting values resulted in a local maximum:
