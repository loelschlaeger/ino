...
knitr::opts_chunk$set(
  ...
  fig.dim = c(8, 6), 
  out.width = "75%",
  # optimizations are pre-computed to save building time
  eval = FALSE
)
devtools::load_all()
# library("ino")
...
- (quasi) Newton (`stats::nlm()`, `stats::nlminb()`, `stats::optim()`), 

- direct search (`pracma::nelder_mead()`), 

- and conjugate gradient methods (`Rcgmin::Rcgmin()`). 

The [CRAN Task View: Optimization and Mathematical Programming](https://CRAN.R-project.org/view=Optimization) provides a comprehensive list of packages for solving optimization problems. 
...
```{r, define em algorithm, eval = TRUE}
...
mixture_ino$
  set_optimizer(optimizer_nlm(), optimizer_label = "nlm")$
  set_optimizer(optimizer_optim(), optimizer_label = "optim")
```
...
```{r, validate mixture_ino, eval = TRUE}
mixture_ino$
  validate(verbose = TRUE, which_direction = "max")
```
...
mixture_ino$optimize(
  which_optimizer = "nlm", save_result = FALSE, return_result = TRUE
)
```
