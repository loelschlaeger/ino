...
How do different optimization approaches perform for this optimization problem? So far, we only applied the `stats::nlm` optimizer. Now, we will compare its results and optimization time to 

1. the `stats::optim` optimizer, an alternative R optimizer, and 
2. the expectation-maximization algorithm, an alternative optimization method for mixture models, which we briefly introduce in the following. 
...
```{r, set em algorithm}
Nop_mixture$set_optimizer(em_optimizer, optimizer_label = "em")
```

```{r, set optim algorithm}
optim <- optimizeR::Optimizer$new(which = "stats::optim")
Nop_mixture$set_optimizer(optim)
```

Clear results, start from same values.

The `$plot()` method provides an overview of the optimization times. Setting `by = "optimizer"` allows for comparison across across optimizers, setting `relative = TRUE` plots relative differences to the median of the top boxplot:

```{r, plot-by-optimizer, eval = FALSE}
Nop_mixture$plot(by = "optimizer", relative = TRUE, xlim = c(-1, 3))
```
