...
The package follows an object-oriented approach^[We make use of the object-oriented framework of the `{R6}` package [@Chang:2021].], treating numerical optimization problems as objects. The object is defined by a real-valued function, the arguments over which the function is to be optimized, and one or more optimization algorithms. The object provides methods for choosing initial values, carrying out numerical minimization or maximization, and evaluating the optimization results.
...
- Generally applicable initialization strategies are readily implemented, and users can extend custom strategies.

- Implemented methods allow for easy comparisons across optimizers and initialization strategies, and to finally extract the best optimization result.

- The package supports parallel computation via the `{future}` package [@Bengtsson:2021] and progress updates via the `{progressr}` package [@Bengtsson:2023].

- We put effort into a user-friendly workflow: every input is documented in detail, every user input is validated, and users only interact with a single object. The package also has a verbose mode, printing hints and status messages, which can be useful to new package users. Activating the verbose mode is described in the appendix below.
